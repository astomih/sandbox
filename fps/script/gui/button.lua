...
local button = function()
  ...
  _.back_drawer = {}
  _.drawer      = {}
  _.bg_color    = Color(0.7, 0.2, 0.2, 1)
  _.fg_color    = Color(1, 1, 1, 0.9)
  ...
  _.show        = function(self, text, pos, scale)
    ...
    if mpos.x >= pos.x - scale.x / 2
        ...
        mpos.y <= pos.y + scale.y / 2
    then
      back_texture:FillColor(Color(0.2, 0.7, 0.2, 1))
      if Mouse.IsPressed(Mouse.LEFT) then
        back_texture:FillColor(Color(0.2, 0.2, 0.7, 1))
        return true
      end
    else
      back_texture:FillColor(self.bg_color)
