      ...
      if self.flag then
        if not self.is_launch then
          if self.timer < self.time then
            ...
            self.texture:FillColor(sn.Color(self.color.r, self.color.g, self.color.b, t))
          else
            self.timer = 0
            self.texture:FillColor(sn.Color(self.color.r, self.color.g, self.color.b, 1.0))
            self.flag = false
            sn.Scene.Change(self.scene_name)
          end
        else
          if self.timer > 0.0 then
            ...
            self.flag = false
            self.texture:FillColor(sn.Color(self.color.r, self.color.g, self.color.b, 0.0))
            self.timer = 0.0
          end
        end
      end
