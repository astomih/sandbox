...
local function camera_controller()
  local object = {
    position = sn.Vec3(0, 0, 0),
    target = sn.Vec3(0, 0, 0),
    up = sn.Vec3(0, 0, 1),
    prev_boost = false,
    ...
    setup = function(self, player)
      self.player = player
      self.position = sn.Vec3(self.player.drawer.position.x,
        self.player.drawer.position.y - self.py,
        self.player.drawer.position.z + self.pz)
      self.target = sn.Vec3(self.player.drawer.position.x,
    ...
    update = function(self)
      ...
      sn.Scene.GetCamera():LookAt(self.position, self.position + self.target, sn.Vec3(0, 0, 1))
    end
  }
  return object
end
