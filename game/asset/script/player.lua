...
Player.update = function(self, map, map_draw3ds, map_size_x, map_size_y)
    ...
    if self.bullet_flag then
        if self.bullet_timer > self.bullet_time and (sn.Mouse.isDown(sn.Mouse.LEFT)) then
            ...
            local cross = sn.Vec3.new(math.cos(math.rad(self.rotationZ + 90)), math.sin(math.rad(self.rotationZ + 90)),
                -0.5)
    ...
    if input_vector.y ~= 0 then
        flag = true
        self.drawer.position = self.drawer.position + sn.Vec3.new(0, input_vector.y * final_speed * sn.Time.delta(), 0)
    end
    if input_vector.x ~= 0 then
        flag = true
        self.drawer.position = self.drawer.position + sn.Vec3.new(input_vector.x * final_speed * sn.Time.delta(), 0, 0)
    end

    if flag then
        local dxy = sn.Vec2.new(self.drawer.position.x, self.drawer.position.y) -
                        sn.Vec2.new(before_pos.x, before_pos.y)
        self.aabb:updateWorld(self.drawer.position - sn.Vec3.new(0, dxy.y, 0), self.drawer.scale, self.model:getAABB())
        ...
        self.aabb:updateWorld(self.drawer.position - sn.Vec3.new(dxy.x, 0, 0), self.drawer.scale, self.model:getAABB())
